package at.klickverbot.takefive.algorithm {
   ...
   public class LineSearcher {
   	public static function findLines( board :IBoard, color :PlayerColor, minLength :int = 2 ) :Array {
   		if ( minLength < 2 ) {
         ...
         for ( var currentCol :int = 0; currentCol < Constants.BOARD_SIZE; ++currentCol ) {
   			lines = lines.concat( search( board, color, minLength,
      ...
      private static function search( board :IBoard, color :PlayerColor,
         minLength :int, startPos :GlobalCoords, direction :GlobalCoords ) :Array {
      	...
      	while ( ( currentPos.x < Constants.BOARD_SIZE ) &&
      	  ( currentPos.y < Constants.BOARD_SIZE ) ) {
      	  	if ( board.readAtGlobal( currentPos ) == color ) {
      	  		++lineLength;
      	  	} else {
      	  		if ( lineLength >= minLength ) {
      	  			result.push( new Line( currentPos, direction, lineLength ) );
               }
      	  		lineLength = 0;
      	  	}
